...
import { Observable, Observer, map, filter, share, Subscription } from 'rxjs';
import { session, Session, Message } from './common/messaging/session';
...
export class RateConsumer implements Observer<RawRate> {
    ...
    public constructor(session: Session, rateStore: RateStore) {
        this.session = session;
        this.store = rateStore;

        const messageStream = this.session.getMessageStream();

        this.monitorStream = messageStream.pipe(filter((m: Message) => {
            return m.topicName === this.PING_TOPIC;
        }));

        this.rateStream = messageStream
            .pipe(filter((m: Message) => {
                return m.topicName !== this.PING_TOPIC;
            }))
            .pipe(map((m: Message) => {
                const r = JSON.parse(m.payload.toString()) as RawRate;

                // TODO: Might not be necessary - I think the rateId might already be populated.
                // TODO: Why is RateId with capital R?
                r.RateId = `${r.ClientId}-${r.Primary}${r.Secondary}${r.Tenor}`;
                r.ingestionTimestamp = m.ingestionTimestamp;
    ...
    public subscribe(topicName: string) : void {
        ...
        this.session.subscribe(topicName);
    }
...
export const rateConsumer = new RateConsumer(session, rateStore);
